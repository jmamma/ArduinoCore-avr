...
private:
  bool readData(void *buf, int count);
  ...
  uint32_t curTimeMS() { return millis(); }
  ...
  //------------------------------------------------------------------------------
  inline uint8_t receive(uint8_t *buf, size_t n) {
    if (n-- == 0) {
      return 0;
    }
    SPDR = 0XFF;
  ...
  enum SDCardCommands {
    ...
    CMD8 = 0x08,   // SEND_IF_COND - verify SD Memory Card interface operating
                   // condition.
    CMD9 = 0x09,   // SEND_CSD - read the Card Specific Data (CSD register)
    CMD10 = 0x0A,  // SEND_CID - read the card identification information (CID
                   // register)
    ...
    ACMD23 = 0x17, // SET_WR_BLK_ERASE_COUNT - Set the number of write blocks to
                   // be pre-erased before writing
    ACMD41 = 0x29, // SD_SEND_OP_COMD - Sends host capacity support information
